...
class DeepCacheSDHelper(object):
    ...
    def is_skip_step(self, block_i, layer_i, blocktype="down"):
        ...
        # Early timesteps: compute normally
        if self.cur_timestep < 5:
            return False

        # Late timesteps: always use cache
        if self.cur_timestep > 15:
            return True

        # Middle timesteps: original DeepCache behavior
        if self.cur_timestep % cache_interval == 0:
            return False
    ...
    def wrap_unet_forward(self):
        ...
        def wrapped_forward(*args, **kwargs):
            ...
            self.cur_timestep = list(
                self.pipe.scheduler.timesteps
            ).index(args[1].item())

            timestep_embedding = args[1].float()
            ...
            if self.prev_embedding is not None:
                ...
                self.change_score = diff.mean().item()

            else:
                self.change_score = 999

            self.prev_embedding = timestep_embedding.clone()
